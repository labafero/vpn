...
const searchGroups = computed(() => [
  {
    ...
    label: 'Navegar',
    items: [
      { id: 'redacao', label: 'Redação', icon: 'i-lucide-pen-line', to: '/redacao' },
      { id: 'novo', label: 'Novo Post', icon: 'i-lucide-plus', to: '/redacao/novo' },
      { id: 'overlay', label: 'Overlay', icon: 'i-lucide-monitor', to: '/overlay', target: '_blank' },
    ],
  },
])
</script>
    ...
    <UDashboardSidebar id="dashboard" v-model:open="open" collapsible resizable>
      <template #header>
        <div class="flex items-center gap-2 px-2 py-1">
          ...
          <span class="text-xl font-bold">VPN</span>
        </div>
      </template>
